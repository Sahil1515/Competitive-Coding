...
bool isSafe(bool visited[][52],int i, int j, int n, int m , vector<string> &arr)
{
    return (i>=0 && i<n && j>=0 && j<m && arr[i][j]=='X' && !visited[i][j]);
    
}

void DFS(bool visited[][52],int i, int j, int n, int m , vector<string> &arr)
{
    int row[4]={0,-1,0,1};
    int col[4]={-1,0,1,0};
    ...
    for(int k=0;k<4;k++)
    {
        if(isSafe(visited,i+row[k],j+col[k],n,m,arr))
        DFS(visited,i+row[k],j+col[k],n,m,arr);
    }
}
...
int Shape(vector<string> &arr,int n,int m) {
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            if(isSafe(visited, i, j,  n, m , arr))
            DFS(visited,i,j,n,m,arr),count++;;
